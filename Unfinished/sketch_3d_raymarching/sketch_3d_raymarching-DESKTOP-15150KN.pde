...
void setup() {
  ...
  model[0] = new Model(cubeObj, new PVector(5, 0, 0));
...
void draw() {
  ...
  for (int y = 0; y < height/pixelSize; y++) {
    for (int x = 0; x < width/pixelSize; x++) {
      ...
      //PVector dirUsing = new PVector(dir[index].x, dir[index].y, dir[index].z); // making a new dir for accomodating reflection
